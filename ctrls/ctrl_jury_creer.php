...
if (isset($_POST["insertJury"])) {
   ...
   $loginJury = htmlentities($_POST["loginJury"]);
   $passwordJury = htmlentities($_POST["passwordJury"]);
   ...
   $loginJury = trim($loginJury);
   $passwordJury = trim($passwordJury);
   $nomJury = trim($nomJury);

   // Vérification de base : si on a saisi quelque chose, c'est gon
   if ($loginJury != "" && $passwordJury != "" && $nomJury != "")
...
if (isset($_POST["updateJury"])) {
   ...
   $jury = new Jury($nomJury, $dateDebutJury);
...
if ($insertJury == True) {
   $jury = new Jury($loginJury, $passwordJury, $nomJury);
   $resInsertJury = insertJury($bdd, $jury);
...
// ---------------------------------------------------------------------------
// cas général
$jurys = selectJurys($bdd); // $jurys: c'est une liste d'objets de la classe hJury
debug($jurys, 'jurys');
...
include("../views/pages/view_jurys_creer.php");
