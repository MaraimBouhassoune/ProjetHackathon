...
if (isset($_POST["insertProjet"])) {
   ...
   $descriptionProjet = htmlentities($_POST["descriptionProjet"]);

   $nomProjet = trim($nomProjet);
   $descriptionProjet = trim($descriptionProjet);
...
if (isset($_POST["updateProjet"])) {
   ...
   // Vérification de base : si on a saisi quelque chose, c'est gon
   if ($nomProjet != "" && $descriptionProjet != "")
      $updateProjet = True; // c'est bon on peut updater
   else {
